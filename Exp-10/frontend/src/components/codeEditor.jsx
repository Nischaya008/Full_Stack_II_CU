...
const CodeEditor = () => {
  ...
  useEffect(() => {
    const fetchRoomDetails = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/api/rooms/${roomId}/details`, {
          headers: {
  ...
  const broadcastCodeUpdate = debounce(async (newCode) => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/rooms/${roomId}/code`, {
        method: 'POST',
        headers: {
      ...
      if (!data.success) {
        ...
        throw new Error(data.message || 'Failed to broadcast code');
      }
  ...
  const broadcastLanguageUpdate = debounce(async (newLanguage) => {
    try {
      ...
      await fetch(`${API_URL}/api/rooms/${roomId}/language`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${userData.token}`
  ...
  const broadcastTerminalsUpdate = debounce(async (newInput, newOutput, loadingState) => {
    try {
      ...
      await fetch(`${API_URL}/api/rooms/${roomId}/terminals`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${userData.token}`
  ...
  const broadcastFileSelection = debounce(async (file) => {
    try {
      ...
      await fetch(`${API_URL}/api/rooms/${roomId}/file-selection`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${userData.token}`
